...
pub struct DataChannel {
    ...
    /// First message already read in server() when peer sent String/Binary (PPID 51/53) instead of DCEP.
    /// Delivered on first read_data_channel() so the message is not lost (e.g. server sends on streamId=0 with ppid=51).
    first_message: Option<Arc<std::sync::Mutex<Option<(Vec<u8>, bool)>>>>,
}
...
impl DataChannel {
    ...
    /// Creates a channel with an initial message already read (e.g. peer sent String/Binary on stream before DCEP).
    fn new_with_first_message(
        stream: Arc<Stream>,
        config: Config,
        first_data: Vec<u8>,
        first_is_string: bool,
    ) -> Self {
        let first_message = Arc::new(std::sync::Mutex::new(Some((first_data, first_is_string))));
        Self {
            config,
            stream,
            messages_sent: Arc::new(AtomicUsize::default()),
    ...
    /// If the peer sends String (PPID 51) or Binary (PPID 53) as the first message instead of DCEP
    /// (e.g. server responds on streamId=0 with ppid=51), the channel is still accepted and the
    /// first message is delivered on the first read_data_channel() call.
    pub async fn server(stream: Arc<Stream>, mut config: Config) -> Result<Self> {
        ...
        if ppi != PayloadProtocolIdentifier::Dcep {
            // Peer sent data (e.g. String ppid=51) before DCEP — accept channel and deliver this message first
            ...
            log::info!(
                "[webrtc-data] incoming stream first message PPID {:?} ({}), accepting channel without DCEP",
                ppi,
                if is_string { "string" } else { "binary" }
            );
            return Ok(DataChannel::new_with_first_message(
                stream,
                config,
                data,
                is_string,
    ...
    pub async fn read_data_channel(&self, buf: &mut [u8]) -> Result<(usize, bool)> {
        // Deliver first message that was read in server() when peer sent String/Binary instead of DCEP
        if let Some(ref fm) = self.first_message {
            if let Ok(mut guard) = fm.lock() {
                if let Some((data, is_string)) = guard.take() {
